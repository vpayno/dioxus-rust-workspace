...
fn HelloListing() -> Element {
    rsx! {
        div { padding: "0.5rem", position: "relative", "hello desktop!" }
    }
}

fn StoryListingOne() -> Element {
    let title = "title";
    let by = "author";
    let score = 0;
    ...
    rsx! {
        div { padding: "0.5rem", position: "relative", "{title} by {by} ({score}) {time} {comments}" }
    }
}

#[component]
fn StoryListingTwo(story: ReadOnlySignal<StoryItem>) -> Element {
    let StoryItem {
        title,
        url,
        by,
...
#[component]
fn StoryListingThree(story: ReadOnlySignal<StoryItem>) -> Element {
    let StoryItem {
        title,
        url,
        by,
